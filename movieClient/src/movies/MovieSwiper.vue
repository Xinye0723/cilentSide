...
function playAudio(chinese, english) {
  ...
  const zhVoice = voices.find(
    (v) =>
      ...
      (!v.gender || v.gender === "female" || v.name.includes("女"))
  );
  // 找出英文女聲
...
<template>
  ...
  >
    <swiper-slide
      ...
      :key="movie.movieId"
      :class="{ 'center-slide': activeIndex === idx }"
    >
      <div
        class="flip-card"
        :class="{ flipped: summaryOpen && activeIndex === idx }"
        @click="onPosterClick(idx)"
      >
        <div class="flip-card-inner">
          ...
          <div
            ...
            :style="{
              background: `linear-gradient(120deg, rgba(32,33,37,0.86) 80%, rgba(44,49,66,0.87) 100%),
                                      url('${
                                        imgBaseUrl + movie.posterPicture
          ...
          >
            ...
            <div class="summary-desc">
              ...
              <span v-if="movie.plot">
                <button
                  class="detail-link"
                  @click.stop="goToDetail(movie.movieId)"
                >
                  詳細介紹
                </button>
              </span>
            ...
            <!-- 語音播放 -->
            <button
              @click="playAudio(movie.movieNameChinese, movie.movieNameEnglish)"
            ></button>
          </div>
        </div>
      </div>
  ...
  <!-- 載入中訊息 -->
  <div v-else style="color: #aaa; text-align: center; padding: 2em">
    載入中...
  </div>
</template>
...
.flip-card-inner {
  width: 100%;
  height: 100%;
  transition: transform 0.55s cubic-bezier(0.6, 0.1, 0.39, 1.3);
...
.flip-card-back {
  background: linear-gradient(
      120deg,
      rgba(32, 33, 37, 0.82) 80%,
      rgba(44, 49, 66, 0.85) 100%
    ),
