...
const isOrganizer = computed(() => {
  ...
  return Number(event.value.organizerId) === Number(localStorage.getItem('memberId'));
});
...
const handleVisibilityChange = async () => {
  if (!document.hidden && event.value) {
    ...
    if (isProcessingPayment.value) {
      ...
      if (memberId) {
        try {
          const updateRes = await fetch(`https://localhost:7181/api/MemberEvent/UpdatePaymentStatus`, {
            method: 'POST',
            headers: {
...
const updateMetaTags = (eventData) => {
  ...
  const metaTags = {
    ...
    'twitter:description': eventData.description || '歡迎參加我們的電影揪團活動！',
...
async function signupOrCancel() {
  ...
  try {
    ...
    console.log(`執行操作: ${action}, 當前狀態: hasSignedUp=${hasSignedUp.value}, isPaid=${isPaid.value}`);
...
const processPayment = async () => {
  ...
  try {
    ...
    // 呼叫綠界付款 API
    const res = await fetch(`https://localhost:7181/api/Ecpay/CreateEventOrder`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
const forceCancelEvent = async () => {
  ...
  // 確認對話框
  const confirmed = confirm('確定要取消此活動嗎？\n\n⚠️ 注意：\n• 活動取消後無法恢復\n• 所有已報名的參加者將被自動取消\n• 已付款的參加者將獲得退款');
  
  if (!confirmed) return;
  
  try {
    const res = await fetch(`https://localhost:7181/api/MemberEvent/cancel/${event.value.memberEventId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      }
...
const shareToFacebook = () => {
  ...
  const description = event.value?.description || '歡迎參加我們的電影揪團活動！';
  ...
  // 使用 Facebook 分享 API
  const shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${encodeURIComponent(shareText)}`;
...
<template>
  <div class="event-detail" v-if="event">
    ...
    <!-- 狀態顯示 -->
    <div class="signup-status" v-if="hasSignedUp && !showPaymentCountdown && !isRedirectingToPayment">
    ...
    <!-- 已付款成功顯示 -->
    <div v-if="hasSignedUp && isPaid && !isRedirectingToPayment" class="success-status">
  ...
  <div v-if="showPaymentCountdown" class="payment-countdown-toast">
    ...
    <div class="countdown-content">
      ...
      <div class="countdown-actions">
        <button @click="payNow" class="pay-now-btn">
          立即付款
        </button>
  ...
  <div v-if="showPaymentModal" class="payment-modal-overlay" @click="closePaymentModal">
    <div class="payment-modal" @click.stop>
      ...
      <div class="payment-actions">
        <button @click="confirmPayment" class="confirm-btn" :disabled="paymentLoading">
...
.back-btn, .signup-btn, .cancel-btn, .organizer-btn, .force-cancel-btn, .share-btn {
  position: relative;
...
.method-option::before {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(90deg, transparent, rgba(163, 135, 255, 0.1), transparent);
...
.payment-total {
  ...
  justify-content: space-between;
  align-items: center;
  background: linear-gradient(135deg, rgba(163, 135, 255, 0.15) 0%, rgba(124, 124, 251, 0.15) 100%);
...
.payment-total::before {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(45deg, transparent 30%, rgba(163, 135, 255, 0.1) 50%, transparent 70%);
