  ...
  try {
    ...
    if (res.ok) {
      ...
      if (hasSignedUp.value !== newHasSignedUp || isPaid.value !== newIsPaid) {
        ...
        console.log(`狀態更新: hasSignedUp=${hasSignedUp.value}, isPaid=${isPaid.value}`);
      }
  ...
  try {
    ...
    if (res.ok) {
      ...
      const statusCheckInterval = setInterval(async () => {
        if (event.value && hasSignedUp.value && !isPaid.value && !showPaymentCountdown.value) {
...
async function executeCancelJoin() {
  ...
  try {
    ...
    const res = await fetch(
      `https://localhost:7181/api/MemberEvent/leave`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const showCancelEventConfirm = () => {
  ...
  confirmModal.style.boxShadow = "0 25px 80px rgba(179, 136, 255, 0.3), 0 10px 40px rgba(0, 0, 0, 0.25)";
  confirmModal.style.backdropFilter = "blur(25px)";
  confirmModal.style.maxWidth = "500px";
  confirmModal.style.fontWeight = "600";
  confirmModal.style.transition = "all 0.5s cubic-bezier(0.68, -0.55, 0.265, 1.55)";
  ...
  confirmModal.style.background = "linear-gradient(135deg, rgba(26, 26, 46, 0.95) 0%, rgba(42, 42, 74, 0.95) 50%, rgba(26, 26, 46, 0.95) 100%)";
  ...
  // 初始狀態
  confirmModal.style.transform = "translate(-50%, -50%) scale(0.3) rotate(-10deg)";
  ...
  // 顯示動畫
  setTimeout(() => {
    confirmModal.style.transform = "translate(-50%, -50%) scale(1) rotate(0deg)";
  ...
  confirmBtn.addEventListener("click", () => {
    // 隱藏確認視窗
    confirmModal.style.transform = "translate(-50%, -50%) scale(0.7) rotate(5deg)";
    confirmModal.style.opacity = "0";
    setTimeout(() => {
...
<template>
  <div class="event-detail" v-if="event">
    ...
    <div
      class="signup-status"
      v-if="hasSignedUp && !showPaymentCountdown && !isRedirectingToPayment && !isPaid"
    >
