...
import { ref } from 'vue';
  ...
  fetch(apiUrl)
    ...
    .then(datas => {
      ...
      nowShowing.value = sorted.filter(movie => movie.movieStatusId === 2).slice(14, 21);
      comingSoon.value = sorted.filter(movie => movie.movieStatusId === 1).slice(0, 7);
    });
...
<template>
  <div class="movie-row">
    ...
    <div class="mt-3">
      <component :is="tabs[activedIndex].component" :movies="activedIndex === 0 ? nowShowing : comingSoon" />
    </div>
  </div>
  <br />
  <br />
